...
class IsoMaxLossFirstPart(nn.Module):
    ...
    def extra_repr(self) -> str:    # new
        return "num_features: " + str(self.num_features) + ", num_classes: " + str(self.num_classes)
...
class IsoMaxLossSecondPart(nn.Module):
    ...
    def extra_repr(self) -> str:    # new
        """Additionally shows entropic_scale when IsoMaxLossSecondPart is printed/represented."""
    ...
    def forward(self, logits, targets, debug=False):
        ...
        if not debug:
            ...
            if torch.cuda.is_available():
                ...
                targets_one_hot = torch.eye(distances.size(1))[targets].long()
                intra_inter_distances = torch.where(targets_one_hot != 0, distances,torch.Tensor([float('Inf')]))
                inter_intra_distances = torch.where(targets_one_hot != 0, torch.Tensor([float('Inf')]), distances)
